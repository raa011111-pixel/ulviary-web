            ...
            <div class="bg-white/80 backdrop-blur-sm overflow-hidden shadow-sm sm:rounded-3xl border border-pink-100/40 p-8">
                <form action="{{ route('categories.store') }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="Contoh: Kuliner, Teknologi, Gaya Hidup" 
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="4" placeholder="Tuliskan deskripsi singkat mengenai kategori ini..." 
                                  class="w-full rounded-xl border border-pink-100 bg-white/50 text-slate-800 placeholder-slate-400 focus:border-rose-400 focus:ring focus:ring-rose-200/50 transition duration-150 @error('description') border-rose-500 @enderror">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
